...
const Customtable = ({ data, columns, invisible, SelectRow, excellReport, cb, setFilteredData, ln }) => {
    ...
    useEffect(() => setFilteredData(table.getFilteredRowModel().rows.map(r => r.original)), [globalFilter, columnFilters])
    ...
    return (
        <div className="flex flex-col relative">
            ...
            {/* SCROLL CONTAINER */}
            <div className="overflow-x-auto overflow-y-auto border-2 border-gray-300 rounded-xl shadow-lg md:max-h-[650px] 2xl:max-h-[850px] relative">
                            ...
                            <Fragment key={group.id}>
                                ...
                                <tr className="bg-blue-50">
                                    {group.headers.map(header => (
                                        <th key={header.id} className="px-5 py-2 text-xs font-semibold text-blue-700 whitespace-nowrap">
                                            {header.column.columnDef.ttlUS}
                                        </th>
                                    ))}
                                </tr>
                                ...
                                {/* Header Row */}
                                <tr className="bg-gradient-to-r from-blue-600 via-blue-700 to-blue-600 shadow-lg">
                                    {group.headers.map(header => (
                                        <th
                                            key={header.id}
                                            className="px-5 py-3 text-left text-xs font-bold text-white uppercase border-r border-white/30 last:border-r-0 whitespace-nowrap"
                                        >
                                            {header.column.getCanSort() ? (
                                    ...
                                    <tr className="bg-white">
                                        {group.headers.map(header => (
                                            <th
                                                ...
                                                className="px-3 py-2"
                                                style={{ zIndex: ['supplier', 'client', 'type'].includes(header.column.id) ? 100 : 50 }}
                                            >
                            ...
                            >
                                {row.getVisibleCells().map(cell => (
                                    <td
                                        key={cell.id}
                                        data-label={cell.column.columnDef.header}
                                        className={`px-4 py-3 text-xs whitespace-nowrap ${styledOutDiv(row, cell)}`}
