...
//             {/* Filter row: centered, fixed max-width inputs to keep spacing consistent */}
//             {filterOn && (
//               <tr className="bg-white relative z-40">
//                 {table.getHeaderGroups()[0].headers.map(header => (
//                   <th key={header.id} className="px-2 py-1 text-left text-xs bg-white">
...
const Customtable = ({
  ...
  return (
    <div className="flex flex-col relative">
            ...
            {table.getHeaderGroups().map(hdGroup =>
              <tr key={hdGroup.id}
                ...
                {hdGroup.headers.map(header =>
                  <th
                    key={header.id}
                    className="relative px-5 py-3 text-left text-xs text-white uppercase 
                    font-semibold hover:bg-[var(--rock-blue)] whitespace-nowrap"
              ...
              <tr className="bg-white border-b-2 border-[var(--selago)]">
                {table.getHeaderGroups()[0].headers.map(header => (
                  <th 
                    ...
                    style={{ position: 'relative', zIndex: header.column.id === 'supplier' ? 100 : 50 }}
                  >
                    {header.column.getCanFilter() ? (
      ...
      {/* ---------- FOOTER ---------- */}
      <div className="table-toolbar flex p-3 border-t border-[var(--selago)] flex-wrap 
        bg-white rounded-b-2xl gap-2 items-center">
