...
//     ? Number(n.replace(/[^0-9.-]+/g, "")) // strip anything not number/dot/minus
//     : Number(n);
...
//   useEffect(() => {
//     const Load = async () => {
//       // Extract year from dateSelect, fallback to current year
...
//   }, [dateSelect, settings]); // Changed from selectedYear to dateSelect
...
//               </div>
//               {/* <div className='flex items-center gap-2 px-4 py-2 bg-[var(--selago)] rounded-xl text-sm text-[var(--port-gore)] font-semibold'>
//                 {dateSelect?.start ? `${dateFormat(dateSelect.start, 'dd-mmm-yy')} - ${dateFormat(dateSelect.end, 'dd-mmm-yy')}` : currentYear}
//               </div> */}
//             </div>
...
//                 ${fmtMoney(Object.values(dataPL).reduce((acc, v) => acc + v, 0) / 1_000_000)}M
...
//                 ${fmtMoney(Object.values(dataInvoices).reduce((acc, v) => acc + v, 0) / 1_000_000)}M
...
//                   ${fmtMoney(
//   (Object.values(dataInvoices).reduce((acc, v) => acc + v, 0) -
//    Object.values(dataPL).reduce((acc, v) => acc + v, 0)) / 1_000_000
// )}M
...
//                 ${fmtMoney(Object.values(dataContracts).reduce((acc, val) => acc + val, 0))}M
...
//                 ${fmtMoney(Object.values(dataExpenses).reduce((acc, val) => acc + val, 0))}M
...
//               </div>
//               <div className='text-2xl font-bold text-white mb-3'>
//                 ${fmtMoney(Object.values(dataInvoices).reduce((acc, val) => acc + val, 0))}M
...
const Dash = () => {
  ...
  return (
    <div className="xl:container mx-auto px-2 md:px-6 xl:px-8 pb-10 md:pb-0 min-h-screen md:pl-[260px]
